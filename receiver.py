...
class Receiver:
    ...
    def absorb_photon(self, photon: Photon, timestamp: int) -> None:
        ...
        '''
        if not (
                photon.absorbed):  # if photon is not initially absorbed, mark it as absorbed.
            photon.got_absorbed()
            self.photons_absorbed += 1

            # Treat the first photon absorbed by each receiver separately:
            if self.photons_absorbed == 1:
                self.activated = True
                self.activation_time = timestamp

            # update total_energy irrespective of nth photon
            # first convert photon's freq to eV
            self.total_energy += Receiver.convert_frequency_to_energy(
                photon.frequency)
        else:
            return None
    ...
    def is_activated(self) -> bool:
        ...
        return self.photons_absorbed > 0
