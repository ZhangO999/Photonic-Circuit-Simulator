...
class Photon:
    ...
    def move(self, board_width: int, board_height: int) -> None:
        ...
        '''
        if self.is_absorbed():
            return  # premature return out to not move
        # supposing photon is not absorbed:
        if self.direction == "N":
            new_y = self.y - 1
        elif self.direction == "S":
            new_y = self.y + 1
        else:  # if direction is E/W, ignore, and ensure that new_y is defined for all cases
            new_y = self.y
        ...
        if 0 <= new_x < board_width and 0 <= new_y < board_height:
            self.x = new_x  # if new positions are within bounds, actually move the photon
            self.y = new_y
        else:  # out-of-bounds -> absorb and don't move (remain in bound)
            self.got_absorbed()

    def interact_with_component(
            self,
            component: object,
            timestamp: int) -> None:
        # only requires implementation once you reach RUN-MY-CIRCUIT
        ...
        '''
        if self.is_absorbed():
            return None  # premature return if photon is already absorbed.
        if component.get_component_type() == "receiver":
            component.absorb_photon(self, timestamp)
            # self.got_absorbed() -> photon absorbed by receiver
