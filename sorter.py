...
def sort_emitters_by_symbol(emitters: list[Emitter]) -> list[Emitter]:
    ...
    ascending order.
    '''
    # copy the emitters objects into a new list so we don't modify the original
    new_list = emitters.copy()

    # perform a bubble sort
    i = 0
    # traverse through all emitters in the list
...
def sort_receivers_by_symbol(receivers: list[Receiver]) -> list[Receiver]:
    ...
    ascending order.
    '''
    # copy the receivers objects into a new list so we don't modify the
    # original
...
def sort_receivers_by_activation_time(
        receivers: list[Receiver]) -> list[Receiver]:
    # this method has already been implemented for you
    '''
    ...
    while i < len(new_list):
        ...
        while j < len(new_list):
            ...
            # swap them if the receiver at the front has a larger activation
            # time
            if new_list[i].get_activation_time(
            ) > new_list[j].get_activation_time():
                temp = new_list[i]
                new_list[i] = new_list[j]
                new_list[j] = temp
            j += 1
...
def sort_receivers_by_total_energy(
        receivers: list[Receiver]) -> list[Receiver]:
    # this method has already been implemented for you
    '''
    This function returns a new list of the same receivers passed in, sorted by
